 ...
 * 업로드 화면
 * - 로컬 팔레트 추출 (현재 방식 유지)
 * - 서버 업로드까지 준비: 서버에 이미지 전송하고 저장 URL/결과 받기
 * - UI: 외곽선 토글 / 색약 필터 토글 / k(클러스터 개수) 조절
 */

export default function Upload() {
  const [imageUrl, setImageUrl] = useState(null)
  const [fileInfo, setFileInfo] = useState(null)
  const [rawFile, setRawFile] = useState(null)        // 실제 File 객체 (서버 전송용)
  ...
  const [saving, setSaving] = useState(false)         // 서버 업로드 중인지
  const [saveResult, setSaveResult] = useState(null)  // 서버가 준 응답 (예: {url, message})

  const dropRef = useRef(null)
  const canvasRef = useRef(null)

  // ====== 백엔드 주소 (필요시 수정) ======
  // 예) 서버가 http://localhost:5000 에서 돌고 있고
  // 업로드 라우트가 POST /api/upload 라고 가정
  const API_BASE = "http://localhost:5000"

  /* ---------------------- 업로드 UI ---------------------- */
  const resetBorder = () => {
    if (dropRef.current) dropRef.current.style.border = '2px dashed #aaa'
  }
  ...
  const onDragOver = (e) => {
    e.preventDefault()
    if (dropRef.current) {
      dropRef.current.style.border = '2px solid #2d7ef7'
    }
  }
  ...
  const uploadToServer = async () => {
    ...
    try {
      ...
      formData.append('image', rawFile) // 서버쪽 multer 필드명이 'image'라고 가정

      // 예: POST http://localhost:5000/api/upload
      const res = await fetch(`${API_BASE}/api/upload`, {
        method: 'POST',
        body: formData,
        // CORS 허용이 안 돼있다면 서버에서 app.use(cors(...)) 필요
      })
      ...
      // data 예: { url: "...", message: "ok" }
      setSaveResult(data)
    } catch (err) {
      console.error(err)
      setSaveResult({ error: '업로드 중 오류가 발생했습니다.' })
    } finally {
      setSaving(false)
    }
  }
            ...
            <div style={{ display:'flex', gap:18, alignItems:'center' }}>
              ...
              <label style={{ userSelect:'none' }}>
                ...
                <input
                  type="checkbox"
                  checked={cbFilter}
                  onChange={(e)=>setCbFilter(e.target.checked)}
                />
              </label>
            </div>
          </div>
            ...
            <div>
              <img
                ...
                style={{
                  ...
                  maxWidth:520,
                  borderRadius:10,
                  border: outline? '2px solid #111' : '1px solid #e5e7eb',
                ...
                <div style={{ marginTop:8, color:'#444', fontSize:14 }}>
                  ...
                  {saveResult && (
                    <div style={{ marginTop:8, fontSize:13, lineHeight:1.4 }}>
                      {saveResult.error
                        ? <span style={{ color:'#dc2626' }}>{saveResult.error}</span>
                        : (
                          <>
                            <div style={{ color:'#16a34a' }}>서버 저장 완료</div>
                            {saveResult.url && (
            ...
            <div>
              <table style={{
                ...
                <tbody>
                  {palette.length === 0 ? (
                    <tr>
                      <td colSpan={5} style={{ padding:'12px 0', color:'#666' }}>
                        팔레트를 계산 중이거나 결과가 없습니다.
                      </td>
                    </tr>
                  ) : (
                    palette.map((c, i) => (
